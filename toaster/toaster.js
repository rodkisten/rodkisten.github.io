...
(function installRodToaster(globalWindow) {
    ...
    const LIGHT_PALETTES = Object.fromEntries(Object.keys(TOAST_COLORS).map((type) => [
        type,
    ...
    const SVG_TEMPLATE_CACHE = new WeakMap();
    const SVG_ICONS = {
    ...
    const ALLOWED_LOADING_ANIMATIONS = new Set(["spinner", "pulse", "progress", "none"]);
    const ALLOWED_POSITIONS = new Set([
        "top-center", "top-left", "top-right", "bottom-center", "bottom-left", "bottom-right",
    ]);
    const ALLOWED_SIZES = new Set(["compact", "comfortable", "large"]);
    const ALLOWED_ACTION_VARIANTS = new Set(["primary", "secondary", "danger", "ghost"]);
    const ALLOWED_TASK_STATUSES = new Set(["queued", "running", "paused", "success", "error", "warning", "cancelled"]);
    ...
    function safePrimitiveText(value, quoteStrings) {
        ...
        if (typeof value === "function")
            return `ƒ ${safeCall(() => value.name || "anonymous", "anonymous")}()`;
        return String(value);
    }
    ...
    function normalizeLoadingAnimation(value) {
        return typeof value === "string" && ALLOWED_LOADING_ANIMATIONS.has(value)
            ? value
            : "spinner";
    }
    ...
    function normalizeToastSize(value) {
        return typeof value === "string" && ALLOWED_SIZES.has(value)
            ? value
            : "compact";
    }
    function normalizeTaskStatus(value) {
        return typeof value === "string" && ALLOWED_TASK_STATUSES.has(value)
            ? value
            : "queued";
    }
    ...
    function getFallbackObjectPreview(value) {
        ...
        if (tag === "[object Map]")
            return `Map(${safeCall(() => value.size, 0)})`;
        if (tag === "[object Set]")
            return `Set(${safeCall(() => value.size, 0)})`;
        if (tag === "[object Date]")
            return safeCall(() => value.toISOString(), "Date");
        if (tag === "[object RegExp]")
            return safeCall(() => String(value), "RegExp");
        if (/Error\]$/.test(tag)) {
    ...
    function applyToastPalette(node, type) {
        ...
        if (node.dataset.rodPalette === paletteKey)
            return;
    ...
    function resolveObjectInspectorSrc() {
        ...
        for (const documentRef of documents) {
            ...
            const candidates = currentScript && String(currentScript.tagName).toUpperCase() === "SCRIPT"
                ? [currentScript, ...scripts.reverse()]
                : scripts.reverse();
    ...
    function getObjectInspectorApi() {
        if (state.inspectorApi)
            return state.inspectorApi;
    ...
    function installInspectorStyle(api) {
        if (!state.renderRoot || !state.hostDocument || state.inspectorStyle?.isConnected)
            return;
    ...
    function setManagerMinimized(minimized) {
        ...
        syncStackLayout();
        if (!state.managerMinimized && count > 1)
    ...
    function syncStackLayout() {
        ...
        for (let sourceIndex = count - 1, stackIndex = 0; sourceIndex >= 0; sourceIndex -= 1, stackIndex += 1) {
            ...
            if (record.options.metadata?.taskId)
                taskCount += 1;
        }
        if (count === 0)
        ...
        if (state.toolbar)
            setDataValue(state.toolbar, "enabled", String(state.config.stackToolbar));
        if (state.stackCountNode) {
            setTextValue(state.stackCountNode, taskCount
        ...
        if (state.managerNode) {
            ...
            if (state.managerNode.title !== title)
                state.managerNode.title = title;
        }
    }
    ...
    function setStackExpanded(expanded) {
        ...
        if (!state.config.stacked || records.length <= 1) {
            ...
            for (const record of records)
                setDataValue(record.node, "itemExpanded", "false");
            syncStackLayout();
            return;
        }
    ...
    function handleStackClick(event) {
        ...
        if (!toastNode || toastNode.dataset.suppressStackClick === "true")
            return;
        const record = getToastRecordByNode(toastNode);
        if (!record)
            return;
        if (!state.stackExpanded) {
            if (state.toasts.length <= 1 || toastNode.dataset.stackIndex !== "0")
                return;
            setStackExpanded(true);
    ...
    function scheduleHostRepair() {
        if (!state.config.persistAcrossSpaNavigation || state.hostRepairFrame !== null)
            return;
        const hostWindow = state.hostWindow ?? initialHostWindow;
    ...
    function installSpaPersistence(hostWindow, hostDocument) {
        if (!state.config.persistAcrossSpaNavigation)
            return;
        // Reinstall when the highest accessible window changes. The original code
        // kept stale listeners and observers in that case.
        if (state.spaCleanup && state.hostWindow !== hostWindow)
            state.spaCleanup();
        if (state.spaCleanup)
            return;
        const callbacks = [];
        const navigationHandler = () => {
            if (state.config.minimizeOnSpaNavigation && hasActiveLoadingRecords())
        ...
        if (typeof hostWindow.MutationObserver === "function" && hostDocument.documentElement) {
            state.spaObserver = new hostWindow.MutationObserver(() => {
                if (state.toasts.length && state.hostElement && !state.hostElement.isConnected)
                    scheduleHostRepair();
            });
    ...
    function ensureHost() {
        ...
        if (!shadowRoot)
            hostElement.dataset.rodToasterFallback = "light-dom";
    ...
    function renderToastValue(value, documentRef, options) {
        if (!isObject(value))
            return renderPrimitive(value, documentRef, false);
        const api = getObjectInspectorApi();
        if (api) {
            state.inspectorApi = api;
    ...
    function normalizeToastOptions(options) {
        const type = typeof options.type === "string" && hasOwn(TOAST_COLORS, options.type)
            ? options.type
            : "default";
    ...
    function removeRecord(record) {
        ...
        if (record.options.id && state.recordsById.get(record.options.id) === record)
            state.recordsById.delete(record.options.id);
        if (record.dedupeKey && state.dedupeRecords.get(record.dedupeKey) === record)
    ...
    function setToastIcon(node, documentRef, iconValue, fallbackName) {
        ...
        const iconName = typeof iconValue === "string" && hasOwn(SVG_ICONS, iconValue)
            ? iconValue
            : fallbackName;
    ...
    function installSwipeToDismiss(record, host) {
        ...
        const snapBack = () => {
            ...
            host.window.setTimeout(() => { if (!record.removed)
                resetVisualState(); }, 240);
        };
        const suppressNextStackClick = () => {
            node.dataset.suppressStackClick = "true";
            host.window.setTimeout(() => { if (node.isConnected)
                delete node.dataset.suppressStackClick; }, 350);
        };
        const finish = (event, cancelled) => {
            ...
            if (capturedPointerId !== null)
                safeCall(() => node.releasePointerCapture(capturedPointerId), undefined);
            const dx = event.clientX - startX;
            const dy = event.clientY - startY;
            const distance = Math.hypot(dx, dy);
            const speed = Math.hypot(velocityX, velocityY);
            const shouldDismiss = !cancelled && mode === "swipe" && (distance >= record.options.swipeThreshold ||
        ...
        node.addEventListener("pointerdown", (event) => {
            if (!record.options.swipeToDismiss || record.removed || !event.isPrimary || event.button > 0 || eventHasInteractiveTarget(event))
                return;
            active = true;
        ...
        node.addEventListener("lostpointercapture", (event) => { if (active && event.pointerId === pointerId)
            finish(event, true); });
    }
    function createToastRecord(args, rawOptions) {
        ...
        const renderLoading = (nextOptions) => {
            ...
            if (progressMeta.hidden !== hideProgressMeta)
                progressMeta.hidden = hideProgressMeta;
        ...
        const cleanup = (reason = dismissReason) => {
            ...
            const dismissEvent = { reason: dismissReason, record, controller, scope: options.scope };
            safeCall(() => options.onDismiss?.(dismissEvent), undefined);
            removeRecord(record);
            state.recordsByNode.delete(node);
            node.remove();
            if (hasEventListeners("dismiss"))
        ...
        const playSuccessExit = () => {
            if (completing || removed || !node.isConnected)
                return;
            completing = true;
            clearTimer();
            setToastIcon(icon, host.document, "check", "check");
            node.dataset.swiping = "false";
            const requestFrame = host.window.requestAnimationFrame?.bind(host.window) ?? ((callback) => host.window.setTimeout(() => callback(performance.now()), 0));
            requestFrame(() => {
                if (!node.isConnected)
                    return cleanup();
        ...
        const dismiss = (immediate = false, swipe = null, reason = "programmatic") => {
            ...
            if (removed || !node.isConnected)
                return cleanup(dismissReason);
            clearTimer();
            if (immediate)
                return cleanup();
            if (!swipe && options.type === "success" && state.config.successExitAnimation)
                return playSuccessExit();
            if (swipe) {
        ...
        const scheduleTimer = () => {
            clearTimer();
            if (removed || paused || !Number.isFinite(remainingDuration) || remainingDuration <= 0)
                return;
            timerStartedAt = Date.now();
            removalTimer = host.window.setTimeout(() => dismiss(false, null, "timeout"), remainingDuration);
        };
        const pauseTimer = () => {
            if (paused || removalTimer === null || !Number.isFinite(remainingDuration) || remainingDuration <= 0)
                return;
            paused = true;
        ...
        const update = (nextArgs, nextRawOptions = {}) => {
            ...
            if (node.getAttribute("role") !== nextOptions.role)
        ...
        controller = {
            ...
            success(...inputArgs) { return settleLoading("success", inputArgs); },
            error(...inputArgs) { return settleLoading("error", inputArgs); },
            info(...inputArgs) { return settleLoading("info", inputArgs); },
            warning(...inputArgs) { return settleLoading("warning", inputArgs); },
            dismiss(reason = "programmatic", immediate = false) {
                if (typeof reason === "boolean")
                    dismiss(reason, null, "programmatic");
                else
        ...
        if (options.closeButton)
            actions.append(createCloseButton(host.document, () => dismiss(false, null, "close")));
        if (options.pauseOnInteraction) {
            ...
            node.addEventListener("focusout", (event) => {
                if (!isDomNode(event.relatedTarget) || !node.contains(event.relatedTarget))
                    resumeTimer();
            });
        }
        ...
        if (hasEventListeners("create")) {
            emitEvent("create", { record, controller, options: { ...options }, args, scope: options.scope });
        }
        installSwipeToDismiss(record, host);
        syncStackLayout();
        const requestFrame = host.window.requestAnimationFrame?.bind(host.window) ?? ((callback) => host.window.setTimeout(() => callback(performance.now()), 0));
        requestFrame(() => { if (node.isConnected)
    ...
    function normalizeActionDescriptors(actions, fallbackActions = []) {
        ...
        return source
            .filter((action) => isUnknownRecord(action))
            .map((action, index) => {
            ...
            return {
                ...
                icon: action.icon === false || action.icon === null
                    ...
                    : typeof action.icon === "string" && hasOwn(SVG_ICONS, action.icon)
                        ? action.icon
                        : null,
                variant: typeof action.variant === "string" && ALLOWED_ACTION_VARIANTS.has(action.variant)
                    ? action.variant
                    : "secondary",
    ...
    function createDetailsNode(documentRef, details, label = "Details") {
        ...
        if (typeof details === "string")
            ...
            body.append(renderToastValue(details, documentRef, state.config));
    ...
    function getToastApi() {
        if (!state.api)
            throw new Error("RodToaster API is not initialized yet.");
        return state.api;
    }
    function showActionDialog(descriptor = {}, settings = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (currentDetailsNode)
                root.append(currentDetailsNode);
            ...
            const setButtonsBusy = (activeButton, action, busy) => {
                ...
                if (activeButton) {
                    ...
                    if (label)
                        label.textContent = busy && action.loadingLabel ? action.loadingLabel : action.label;
                }
            };
            const executeAction = async (action, button, event) => {
                if (settled || button.disabled)
                    return;
                setValidation("");
                ...
                if (currentSvg)
                    currentSvg.replaceWith(loader);
                else
                    button.insertBefore(loader, labelNode);
                ...
                try {
                    emitEvent("action", { phase: "start", actionId: action.id, action: action.raw, controller, scope: options.scope ?? null });
                    ...
                    emitEvent("action", { phase: "success", actionId: action.id, action: action.raw, result, controller, scope: options.scope ?? null });
                    if (action.close) {
                    ...
                    if (originalIcon)
                        button.insertBefore(originalIcon, labelNode);
                    if (labelNode)
                        labelNode.textContent = action.label;
                    setButtonsBusy(button, action, false);
                }
                catch (error) {
                    emitEvent("action", { phase: "error", actionId: action.id, action: action.raw, error, controller, scope: options.scope ?? null });
            ...
            normalizedActions.forEach((action) => {
                ...
                if (action.icon)
                    button.append(createSvgIcon(documentRef, action.icon, 15));
            ...
            const updateCountdown = () => {
                ...
                buttons.forEach((button) => {
                    ...
                    if (label && button.dataset.busy !== "true")
                        label.textContent = action.labelTemplate.replace(/\{seconds\}/g, String(remainingSeconds));
            ...
            normalizedActions.forEach((action) => { if (action.shortcut)
    ...
    function showPromptToast(descriptor = {}) {
        ...
        return showActionDialog(options, {
            ...
            buildBody({ document, body }) {
                ...
                if (Number.isFinite(options.minLength))
                    input.minLength = Number(options.minLength);
        ...
        return showActionDialog(options, {
            ...
            buildBody({ document, body }) {
                ...
                choices.forEach((choice, index) => {
                    const choiceDescriptor = isUnknownRecord(choice) ? choice : { value: choice, label: choice };
    ...
    function showRichToast(descriptor = {}, forcedType = null) {
        ...
        const render = (next = options) => {
            ...
            if (next.copyError !== false && next.error) {
                ...
                if (copyAction)
                    normalizedActions.push(copyAction);
            }
            normalizedActions.forEach((action) => {
                const button = documentRef.createElement("button");
                const label = documentRef.createElement("span");
                ...
                button.disabled = action.disabled;
                if (action.icon)
                    button.append(createSvgIcon(documentRef, action.icon, 14));
                label.textContent = action.label;
                button.append(label);
                button.addEventListener("click", (event) => {
                    event.preventDefault();
                    ...
                    if (action.loadingLabel)
                        label.textContent = action.loadingLabel;
                    void Promise.resolve(action.handle
                        ...
                        if (action.successLabel)
                            label.textContent = action.successLabel;
                        if (action.close) {
                            controller.dismiss("action");
                            return;
                        }
    ...
    function flushTaskSnapshots() {
        ...
        for (const task of state.tasks.values()) {
            if (!task.persist || task.dismissed)
                continue;
    ...
    function createTaskController(descriptor = {}) {
        ...
        const renderTaskActions = () => {
            ...
            if (signature === lastTaskActionsSignature)
                return;
            lastTaskActionsSignature = signature;
            taskActions.replaceChildren();
            const descriptors = [];
            if (options.pausable && (taskState.status === "running" || taskState.status === "queued"))
            ...
            if (Array.isArray(options.actions))
                descriptors.push(...options.actions.map((action) => ({ ...action, close: action.close === true })));
            normalizeActionDescriptors(descriptors).forEach((action) => {
                ...
                button.dataset.busy = "false";
                if (action.icon)
                    button.append(createSvgIcon(node.ownerDocument, action.icon, 14));
                label.textContent = action.label;
                button.append(label);
                button.addEventListener("click", (event) => {
                    event.preventDefault();
        ...
        const apply = (next = {}, emit = true) => {
            if (dismissed)
                return task;
            if (hasOwn(next, "title"))
                taskState.title = String(next.title ?? "");
            if (hasOwn(next, "description"))
                taskState.description = String(next.description ?? "");
            if (hasOwn(next, "icon"))
                taskState.icon = next.icon === false ? false : next.icon ?? "circle";
            if (hasOwn(next, "status"))
                taskState.status = normalizeTaskStatus(next.status);
            if (hasOwn(next, "progress"))
                taskState.progress = normalizeProgress(next.progress);
            if (hasOwn(next, "progressLabel"))
                taskState.progressLabel = next.progressLabel == null ? null : String(next.progressLabel);
            if (isUnknownRecord(next.metadata))
            ...
            if (emit && hasEventListeners("task:update")) {
                emitEvent("task:update", { task, snapshot: task.snapshot() });
            }
            return task;
        };
        Object.assign(task, {
            ...
            start: (next = {}) => apply({ ...next, status: "running" }),
            setProgress: (value, next = {}) => apply({ ...next, status: next.status ?? "running", progress: value }),
            pause: async () => {
                if (dismissed || paused)
                    return task;
            ...
            cancel: async (reason = "cancelled") => {
                if (dismissed || ["success", "error", "cancelled"].includes(taskState.status))
                    return task;
    ...
    function resolvePhaseDescriptor(spec, value, fallback = {}) {
        ...
        if (typeof resolved === "string")
            return { ...fallback, description: resolved };
    ...
    async function showRetryToast(descriptor) {
        ...
        if (!task)
            throw new Error("Could not create retry task.");
    ...
    function createTaskGroup(descriptor = {}) {
        ...
        if (!parent)
            throw new Error("Could not create task group parent.");
        const group = {
            id,
            parent,
            children,
        ...
        group.unsubscribe = addEventListenerInternal("task:update", (event) => {
            ...
            if (!isTaskController(candidateTask) || candidateTask === parent)
                return;
            if (candidateTask.snapshot().metadata.groupId === id)
                group.recompute();
        });
    ...
    function restorePersistedTasks() {
        if (!state.config.persistTasks || state.restoredTasks)
            return [];
        ...
        getPersistedTaskSnapshots().forEach((snapshot) => {
            ...
            if (terminal && now - Number(snapshot.updatedAt || 0) > state.config.taskTerminalRetention)
                return;
    ...
    toastApi.off = (eventName, listener) => {
        ...
        if (!bucket.size)
            state.listeners.delete(String(eventName || "*"));
        return removed;
    };
    ...
    toastApi.configure = (nextConfig = {}) => {
        ...
        if (!state.config.persistAcrossSpaNavigation)
            state.spaCleanup?.();
        else if (state.hostWindow && state.hostDocument)
            installSpaPersistence(state.hostWindow, state.hostDocument);
        const hostModeChanged = previousHostConfig.useShadowRoot !== state.config.useShadowRoot ||
