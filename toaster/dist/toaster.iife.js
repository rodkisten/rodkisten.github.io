/* Auto-generated from toaster/toaster.ts. at 8/24/2026, 10:54:24 AM Do not edit directly. */
...
//#region toaster/toaster.ts
	...
	(function installRodToaster(globalWindow) {
		...
		function normalizeExpandedDescriptor(value) {
			...
			if (isPlainObject(value)) {
				if ([
					...
					"defaultOpen",
					"maxHeight"
				].some((key) => hasOwn(value, key))) return {
					content: value.content,
					title: value.title,
					defaultOpen: Boolean(value.defaultOpen),
					maxHeight: typeof value.maxHeight === "number" || typeof value.maxHeight === "string" ? value.maxHeight : void 0
				};
			}
    ...
    `;
		...
		function createContentExpandButton(documentRef, toggle) {
			const button = documentRef.createElement("button");
			button.type = "button";
		...
		function createToastRecord(args, rawOptions) {
			...
			let expandedInner = null;
			let expandedHeading = null;
			let expandedScroll = null;
			...
			const pauseTimer = (reason = "interaction") => {
				if (pauseReasons.has(reason)) return;
				const wasPaused = pauseReasons.size > 0;
				pauseReasons.add(reason);
				if (wasPaused) return;
				if (removalTimer !== null && Number.isFinite(remainingDuration) && remainingDuration > 0) remainingDuration = Math.max(0, remainingDuration - (Date.now() - timerStartedAt));
				clearTimer();
			};
			const resumeTimer = (reason = "interaction") => {
			...
			const ensureExpandedNodes = () => {
				if (expandedPanel && expandedInner && expandedHeading && expandedScroll) return {
					panel: expandedPanel,
					inner: expandedInner,
					heading: expandedHeading,
					scroll: expandedScroll
				};
			...
			const setContentExpanded = (open) => {
				contentExpanded = Boolean(options.expanded) && Boolean(open);
				...
				if (contentExpanded) {
					pauseTimer("expanded");
					if (state.stackExpanded && getStackRecords().length > 1 && record && !record.removed) setExpandedToast(record, true);
				} else resumeTimer("expanded");
			};
			const renderExpandedContent = (descriptor) => {
				...
				const maxHeight = typeof rawMaxHeight === "number" ? `${Math.max(80, rawMaxHeight)}px` : typeof rawMaxHeight === "string" && rawMaxHeight.trim() ? rawMaxHeight.trim() : "min(42dvh, 360px)";
