/* Auto-generated from menu/menu.ts. at 8/19/2026, 11:46:32 AM Do not edit directly. */
...
`;
		...
		class SurfaceController {
			...
			revealActiveTab(nav, behavior = "smooth") {
				const tabs = nav || this.root.querySelector(".rm-tabs");
				if (!tabs || !this.activeTabId) return;
				const escaped = typeof CSS !== "undefined" && typeof CSS.escape === "function" ? CSS.escape(this.activeTabId) : this.activeTabId.replace(/["\\]/g, "\\$&");
				const active = tabs.querySelector(`.rm-tab[data-tab-id="${escaped}"]`);
				if (!active) return;
				const maxScroll = Math.max(0, tabs.scrollWidth - tabs.clientWidth);
				const desired = Math.max(0, Math.min(maxScroll, active.offsetLeft - 2));
			...
			*/
			renderTypeaheadSelect(field, value, multiple) {
				...
				const selected = new Set(multiple ? Array.isArray(value) ? value.map(String) : [] : [String(value ?? "")].filter(Boolean));
				let query = "";
				let open = false;
				...
				const filteredOptions = () => {
					...
					return Array.from(field.options).filter((option) => {
						return `${option.label} ${option.value} ${option.description || ""}`.toLocaleLowerCase().includes(normalized);
					});
				};
				...
				const scrollHighlightedIntoView = () => {
					list.querySelector(`.rm-typeahead-option[data-index="${highlightedIndex}"]`)?.scrollIntoView({ block: "nearest" });
				};
				...
				const renderOptions = () => {
					...
					options.forEach((option, index) => {
						...
						button.addEventListener("mouseenter", () => {
							...
							for (const item of Array.from(list.querySelectorAll(".rm-typeahead-option"))) item.dataset.highlighted = String(item === button);
						});
				...
				input.addEventListener("keydown", (event) => {
					...
					if (event.key === "Enter" && open && options.length) {
						event.preventDefault();
						list.querySelector(`.rm-typeahead-option[data-index="${highlightedIndex}"]`)?.click();
						return;
					}
				...
				const onRootPointerDown = (event) => {
					if (!open) return;
					if ((typeof event.composedPath === "function" ? event.composedPath() : []).includes(wrap)) return;
					closeList();
				};
			...
			setActiveTab(tabId) {
				...
				requestAnimationFrame(() => {
					if (this.destroyed) return;
					const nav = wrapper.querySelector(".rm-tabs");
